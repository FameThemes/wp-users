...
jQuery(document).ready(function($) {
    ...
    $('.wpu-singup-btn, .wpu-login-btn').click( function( event ) {
	...
	function __init( w ) {
        ...
        var $form_modal = ( $('.wpu-modal' , w).not('.wpu-loaded').length >0 )  ?  $('.wpu-modal' , w).not('.wpu-loaded') :  $('.wpu-modal').not('.wpu-loaded'),
        ...
        $form_modal.addClass('wpu-loaded');
        ...
        $('.wpu-modal').on('click', function(event) {
            if ( $form_modal.hasClass('wpu-disabled') ) {
                return false;
            }
